...
public class HeditController {
    ...
    public void init(Manager manager1, HouseholderInformation selectedItem, AdministratorController administratorController1, Stage stage1) {
    ...
    private void confirm(){
        ...
        if (!id.equals("") && AddController.isNumeric(tel) && !name.equals("") && !wechat.equals("")) {
            manager.insertH(new HouseholderInformation(id,name,sex,tel,wechat));
            stage.close();
            administratorController.Rrefresh();
        } else {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error");
            alert.setHeaderText("房东信息缺失或格式问题");
            alert.setContentText("请检查您的房东信息格式与完整性");
            alert.showAndWait();
        }
    }
}
